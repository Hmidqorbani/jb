...
app.get("/", (req, res) => {
  res.render("game", {
    title: "JERESH BEDE",
    author: "hamidqorbni",
    githubUrl: "https://github.com/Hmidqorbani",
  });
});

// Create project structure
function setupProject() {
  const dirs = ["views", "public", "public/css", "public/js"];
...
<body>
    ...
    <div class="menus">
        <div class="menu menu--main">
            ...
            <div class="credits">game by <a href="<%= githubUrl %>"><%= author %></a></div>
        </div>
...
@media (min-width: 1025px) {
  ...
  // Create JavaScript file with complete game logic
  const jsContent = `// globalConfig.js
let gameSpeed = 1;
...
function tick(width, height, simTime, simSpeed, lag) {
    ...
    for (let i = frags.length - 1; i >= 0; i--) {
        ...
        if (frag.projected.y > centerY + targetHitRadius ||
...
setupCanvases();`;
  ...
  // Create package.json
  const packageJson = {
    name: "jeresh-bede-game",
    version: "1.0.0",
    description: "Jeresh Bede - A 3D cube smashing game",
    main: "game-server.js",
    scripts: {
    ...
    keywords: ["game", "3d", "webgl", "nodejs"],
    author: "hamidqorbni",
    license: "MIT",
  };

  // Write all files
  fs.writeFileSync("./views/game.ejs", ejsTemplate);
  fs.writeFileSync("./public/css/style.css", cssContent);
  fs.writeFileSync("./public/js/script.js", jsContent);
  fs.writeFileSync("./package.json", JSON.stringify(packageJson, null, 2));

  console.log("✅ Project setup complete!");
  console.log("📁 Directories created: views/, public/css/, public/js/");
  console.log("📄 Files created:");
  console.log("   - views/game.ejs");
  ...
  console.log("\\n🎮 Game features:");
  console.log("   - 3D cube smashing gameplay");
  console.log("   - Multiple game modes");
  console.log("   - Slow-mo power-ups");
...
if (require.main === module) {
  ...
  app.listen(port, () => {
    console.log(
      `\\n🎯 Jeresh Bede game server running at http://localhost:${port}`
    );
  });
}
